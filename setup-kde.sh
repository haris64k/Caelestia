...
# Caelestia KDE Plasma Setup Script
# This script sets up the KDE Plasma version of Caelestia dotfiles
...
log "Caelestia KDE Plasma Setup"
...
# Install optional applications if requested
if [[ "$1" == "--full" ]]; then
    ...
    if command -v yay &> /dev/null; then
        ...
        warning "No AUR helper found. Skipping optional applications."
        AUR_HELPER=""
    fi
    ...
    if [[ -n "$AUR_HELPER" ]]; then
        ...
        $AUR_HELPER -S --needed adw-gtk-theme papirus-icon-theme sweet-cursors ttf-jetbrains-mono-nerd
        
        # Install applications
        log "Installing applications..."
        $AUR_HELPER -S --needed fish foot fastfetch btop starship
        
        success "Optional applications installed"
    fi
fi
...
echo "3. Install Bismuth for tiling: $AUR_HELPER -S kwin-bismuth"
